...
void GameView::addQuad(float cx, float cy, float w, float h, const float color[3]) {
    for (int i = 0; i < 6; i++) {
        float dx = (i == 1 || i == 3 || i == 4) ? w : -w;
        float dy = (i == 0 || i == 1 || i == 4) ? -h : h;
        _v[0] = cx + dx; _v[1] = cy + dy;
        _v[2] = color[0]; _v[3] = color[1]; _v[4] = color[2];
        _v += 5;
    }
}

void GameView::renderCellItems(float cx, float cy, int count, const float color[3], float scale) {
    if (count <= 0) return;
    float itemDotSize = 0.05f * scale;
    float spacing = itemDotSize * 2.5f;
    switch (count) {
        case 1: addQuad(cx, cy, itemDotSize, itemDotSize, color); break;
        case 2: addQuad(cx - spacing * 0.5f, cy, itemDotSize, itemDotSize, color); addQuad(cx + spacing * 0.5f, cy, itemDotSize, itemDotSize, color); break;
        case 3: addQuad(cx, cy - spacing * 0.5f, itemDotSize, itemDotSize, color); addQuad(cx - spacing * 0.5f, cy + spacing * 0.5f, itemDotSize, itemDotSize, color); addQuad(cx + spacing * 0.5f, cy + spacing * 0.5f, itemDotSize, itemDotSize, color); break;
        case 4: addQuad(cx - spacing * 0.5f, cy - spacing * 0.5f, itemDotSize, itemDotSize, color); addQuad(cx + spacing * 0.5f, cy - spacing * 0.5f, itemDotSize, itemDotSize, color); addQuad(cx - spacing * 0.5f, cy + spacing * 0.5f, itemDotSize, itemDotSize, color); addQuad(cx + spacing * 0.5f, cy + spacing * 0.5f, itemDotSize, itemDotSize, color); break;
        case 5: addQuad(cx, cy, itemDotSize, itemDotSize, color); addQuad(cx - spacing * 0.5f, cy - spacing * 0.5f, itemDotSize, itemDotSize, color); addQuad(cx + spacing * 0.5f, cy - spacing * 0.5f, itemDotSize, itemDotSize, color); addQuad(cx - spacing * 0.5f, cy + spacing * 0.5f, itemDotSize, itemDotSize, color); addQuad(cx + spacing * 0.5f, cy + spacing * 0.5f, itemDotSize, itemDotSize, color); break;
        default: addQuad(cx, cy, itemDotSize, itemDotSize, color); break;
    }
}

void GameView::renderGrid(int nodeIndex, float ox, float oy, float width, float pitch, int depth) {
    ...
    if (half < 0.001f) half = pitch * 0.45f;
    cellSize = half * 2.0f;
        ...
        for (int c = 0; c < size; c++) {
            ...
            if (depth + 1 < MAX_PREVIEW_DEPTH && pitch > _gap * 3.0f) {
                ...
                if (childNode.type == CellType::GRID) {
                    ...
                    renderCell(childIndex, childCx - childWidth * 0.5f, childCy - childWidth * 0.5f, childWidth, childPitch, 0, nullptr, depth + 1);
                    continue;
                }
            }
            renderCell(childIndex, childCx - half, childCy - half, 0, 0, cellSize, bg, depth + 1);
        }
    }
}

void GameView::renderCell(int nodeIndex, float ox, float oy, float width, float pitch, float cellSize, const float bg[3], int depth) {
    const auto& cell = _model.node(nodeIndex);
    ...
    if (cell.type == CellType::EMPTY) {
        ...
        renderCellItems(ox + half, oy + half, cell.data.item.count, col, pitch > 0 ? pitch / _cellSize : 1.0f);
